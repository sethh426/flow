 ...
 */
/**
 * Connection & Behavior Notes (Added for reliability):
 * - WebSocket URL resolution order:
 *     1. process.env.NEXT_PUBLIC_FLOW_ORCHESTRATOR_WS
 *     2. ws://localhost:8080/flow-autopilot (dev localhost)
 *     3. Placeholder production wss://flow-orchestrator-xxxxx-uc.a.run.app/flow-autopilot
 * - Exponential backoff reconnect (1s,2s,4s,... max 15s) while enabled=true.
 * - Component is visually dormant when not enabled (renders null early).
 * - Public props:
 *      enabled?: boolean   -> start/stop the autonomous agent & socket lifecycle
 *      onStatusChange?: fn -> receives 'connecting' | 'connected' | 'retrying' | 'error' | 'disconnected'
 * - UI indicators: bottom-left Chip + Snackbar to surface connection lifecycle to user.
 * - Safety: prevents duplicate WebSocket creation; cleans up timers & socket on unmount.
 * - Extension ideas: command queue, pause/resume control, bandwidth throttling, auth token injection.
 */

import { useState, useEffect, useCallback, useRef } from 'react';
import { Box, Typography, Fade, Zoom, Chip, Snackbar, Alert } from '@mui/material';
...
export default function FlowAutopilot({ enabled = false, onStatusChange }: FlowAutopilotProps) {
  ...
  const [connectionStatus, setConnectionStatus] = useState<'idle' | 'connecting' | 'connected' | 'error' | 'disconnected' | 'retrying'>('idle');
  ...
  // Show thought bubble
  const showThought = useCallback((message: string) => {
    setCurrentThought(message);
    setTimeout(() => setCurrentThought(null), 3000);
  }, []);

  // Fly to an element
  const flyToElement = useCallback(async (selector: string): Promise<void> => {
  ...
  // Click an element
  const clickElement = useCallback(async (selector: string): Promise<void> => {
  ...
  }, [flyToElement, showThought, position]);

  // Navigate to a route
  const navigateTo = useCallback(async (route: string): Promise<void> => {
  ...
  const celebrate = useCallback(async (message?: string): Promise<void> => {
  ...
  }, [showThought, celebrate, clickElement, flyToElement, navigateTo]);
  ...
  useEffect(() => {
    ...
    const wsUrl = process.env.NEXT_PUBLIC_FLOW_ORCHESTRATOR_WS 
      || (typeof window !== 'undefined' && window.location.hostname === 'localhost'
        ...
        : process.env.NEXT_PUBLIC_FLOW_ORCHESTRATOR_WS || 'ws://localhost:8080/flow-autopilot'); // Fallback to localhost if not configured
  ...
  }, [enabled, onStatusChange, handleCommand]);
  ...
  return (
    <>
      ...
      <Box sx={{ position: 'fixed', bottom: 12, left: 12, zIndex: 1400 }}>
        <Chip
          label={
            connectionStatus === 'connected'
              ...
              : connectionStatus === 'connecting'
                ...
                : connectionStatus === 'retrying'
                  ? `Retrying (${retryCountRef.current})…`
      ...
      >
        <Alert
          ...
          sx={{ width: '100%' }}
        >
          {connectionStatus === 'connected' && '🤖 Flow Autopilot connected'}
          {connectionStatus === 'connecting' && '🔌 Connecting to Flow Orchestrator…'}
          {connectionStatus === 'retrying' && `🔄 Retrying connection (attempt ${retryCountRef.current})…`}
