...
interface Grid2Props {
  ...
  sx?: React.CSSProperties; // Material-UI sx prop
  [key: string]: unknown; // For other props passed through
}

const Grid2 = ({ children, container, spacing, size, ...props }: Grid2Props) => {
  if (container) {
...
export default function ContentStudioPremium() {
  ...
  const [aiSuggestions] = useState(AI_SUGGESTIONS);
...
Each set should have 4-6 trending hashtags. Return as a simple numbered list.`;
  ...
  const exportContent = async (format: 'png' | 'jpg' | 'webp' | 'pdf' | 'svg') => {
    ...
    try {
      ...
      tempCanvas.width = selectedTemplate.size.width;
      tempCanvas.height = selectedTemplate.size.height;
      ...
      ctx.font = `${content.fontWeight} ${content.fontSize}px system-ui, -apple-system, sans-serif`;
      ctx.textAlign = content.textAlign;

      const xPos = content.textAlign === 'center' ? tempCanvas.width / 2 :
                   content.textAlign === 'right' ? tempCanvas.width - 40 : 40;

      // Draw title
      if (content.title) {
        ctx.font = `700 ${content.fontSize + 8}px system-ui, -apple-system, sans-serif`;
        ctx.fillText(content.title, xPos, 100);
      }
      ...
      if (content.description) {
        ...
        lines.forEach((line, i) => {
          ctx.fillText(line, xPos, 150 + (i * (content.fontSize + 10)));
        });
      }
      ...
      tempCanvas.toBlob((blob) => {
        ...
        const filename = `${content.title.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${Date.now()}.${format}`;
      ...
      <Box sx={{ 
        ...
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
          <Button
            ...
            data-testid="pro-tools-button"
            aria-label="Open professional tools"
          >
            Pro Tools
          </Button>
          <Button
                ...
                {filteredTemplates.map((template) => (
                  <Card
                    ...
                    sx={{
                      ...
                      borderColor: selectedTemplate.id === template.id ? 'primary.main' : 'rgba(0, 0, 0, 0.08)',
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ mt: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="body2" fontWeight={600} color="text.secondary">
                  {selectedTemplate.name} • {selectedTemplate.size.width}x{selectedTemplate.size.height}px
                </Typography>
          ...
          <Card sx={{ 
            ...
            <CardContent sx={{ p: 2.5 }}>
              <Tabs
                ...
                variant="fullWidth"
                data-testid="content-studio-tabs"
                aria-label="Content studio navigation tabs"
              >
                <Tab label="Content" sx={{ minHeight: 48 }} data-testid="content-tab" />
                <Tab label="Style" sx={{ minHeight: 48 }} />
              </Tabs>
              ...
              {currentTab === 0 && (
                <>
                  <Box>
                    <Typography variant="caption" fontWeight={600} color="text.secondary" gutterBottom display="block" sx={{ mb: 1 }}>
                      CONTENT
                    </Typography>
                    <TextField
                      fullWidth
                  ...
                  <Box>
                    <Typography variant="caption" fontWeight={600} color="text.secondary" gutterBottom display="block" sx={{ mb: 1.5 }}>
                      MEDIA
                    </Typography>
                    <Button
                      fullWidth
                  ...
                  <Box>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      ...
                      <IconButton
                        size="medium"
                        sx={{
                          border: content.fontWeight === 'bold' ? '2px solid' : '1px solid',
                          borderColor: content.fontWeight === 'bold' ? '#667eea' : 'rgba(0, 0, 0, 0.12)',
                          background: content.fontWeight === 'bold' ? 'rgba(102, 126, 234, 0.1)' : 'transparent',
                          '&:hover': {
                            borderColor: '#667eea',
                            background: 'rgba(102, 126, 234, 0.1)',
                          },
                        ...
                        aria-pressed={content.fontWeight === 'bold'}
                      >
                        <FormatBold />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Align Left">
                      <IconButton
                        size="medium"
                        sx={{
                          border: content.textAlign === 'left' ? '2px solid' : '1px solid',
                          borderColor: content.textAlign === 'left' ? '#667eea' : 'rgba(0, 0, 0, 0.12)',
                          background: content.textAlign === 'left' ? 'rgba(102, 126, 234, 0.1)' : 'transparent',
                          '&:hover': {
                            borderColor: '#667eea',
                            background: 'rgba(102, 126, 234, 0.1)',
                          },
                        ...
                        aria-pressed={content.textAlign === 'left'}
                      >
                        <FormatAlignLeft />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Align Center">
                      <IconButton
                        size="medium"
                        sx={{
                          border: content.textAlign === 'center' ? '2px solid' : '1px solid',
                          borderColor: content.textAlign === 'center' ? '#667eea' : 'rgba(0, 0, 0, 0.12)',
                          background: content.textAlign === 'center' ? 'rgba(102, 126, 234, 0.1)' : 'transparent',
                          '&:hover': {
                            borderColor: '#667eea',
                            background: 'rgba(102, 126, 234, 0.1)',
                          },
                        ...
                        aria-pressed={content.textAlign === 'center'}
                      >
                        <FormatAlignCenter />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Align Right">
                      <IconButton
                        size="medium"
                        sx={{
                          border: content.textAlign === 'right' ? '2px solid' : '1px solid',
                          borderColor: content.textAlign === 'right' ? '#667eea' : 'rgba(0, 0, 0, 0.12)',
                          background: content.textAlign === 'right' ? 'rgba(102, 126, 234, 0.1)' : 'transparent',
                          '&:hover': {
                            borderColor: '#667eea',
                            background: 'rgba(102, 126, 234, 0.1)',
                          },
          ...
          <Grid2 container spacing={2}>
            {filteredMediaAssets.length === 0 ? (
              ...
              filteredMediaAssets.map((asset) => (
                <Grid2 size={{ xs: 12, md: 4, lg: 3 }} key={asset.id}>
                  <Card 
            ...
            <Box>
              ...
              <Grid2 container spacing={1}>
                {[
                  ...
                  <Grid2 size={{ xs: 12, md: 6 }} key={item.platform}>
                ...
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    <Button 
                      ...
                      data-testid="auto-enhance-image-button"
                    ...
                    <Button 
                      ...
                      data-testid="professional-typography-button"
                      aria-label="Apply professional typography"
                    >
                    ...
                    <Button 
                      ...
                      data-testid="apply-gradient-button"
